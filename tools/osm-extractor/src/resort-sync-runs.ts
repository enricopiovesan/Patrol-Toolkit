...
export type ResortSyncRunsResult = {
...
export function buildRunsOverpassQuery(args: {
  minLon: number;
  minLat: number;
  maxLon: number;
  maxLat: number;
...
(
  way["piste:type"="downhill"](${args.minLat},${args.minLon},${args.maxLat},${args.maxLon});
  relation["piste:type"="downhill"](${args.minLat},${args.minLon},${args.maxLat},${args.maxLon});
);
out geom tags;`;
}

export async function syncResortRuns(
  args: {
    workspacePath: string;
    outputPath?: string;
    bufferMeters?: number;
  ...
  const outputPath = args.outputPath ?? join(dirname(args.workspacePath), "runs.geojson");

  await writeResortWorkspace(
    args.workspacePath,
    withLayerState(workspace, "runs", {
      status: "running",
      queryHash,
      updatedAt,
      error: undefined
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Runs sync failed: Overpass returned HTTP ${response.status}.`);
    }

    const raw = (await response.json()) as { elements?: OverpassElement[] };
    const features = toRunFeatures(raw.elements ?? []);
    ...
    await writeResortWorkspace(
      args.workspacePath,
      withLayerState(completedWorkspace, "runs", {
        status: "complete",
    ...
    await writeResortWorkspace(
      args.workspacePath,
      withLayerState(failedWorkspace, "runs", {
        status: "failed",
        queryHash,
        updatedAt,
        error: message
...
function toRunFeatures(elements: OverpassElement[]): Array<{
  type: "Feature";
  geometry: { type: "LineString"; coordinates: [number, number][] };
  properties: { id: string; name: string; difficulty: string | null };
}> {
  return elements
    .filter((element) => element.type === "way")
    .map((element) => {
      const pisteType = element.tags?.["piste:type"]?.trim();
      ...
      return {
        ...
        properties: {
          ...
          name: element.tags?.name?.trim() || `Run ${String(element.id ?? "unknown")}`,
          difficulty: element.tags?.["piste:difficulty"] ?? null
        }
      };
    })
